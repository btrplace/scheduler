 ...
 */
public class LightPacking extends Packing {
     ...
     */
    public LightPacking(ReconfigurationProblem p) {
        loads = new ArrayList<>();
        bins = new ArrayList<>();
        sizes = new ArrayList<>();
        names = new ArrayList<>();
...
//            }
        ...
        if (!rp.getFutureRunningVMs().isEmpty()) {
            solver.post(new VectorPacking(new String[]{"foo"}, loads.toArray(new IntVar[loads.size()][]), iSizes, bins.get(0)));
        }
        return true;
    }
