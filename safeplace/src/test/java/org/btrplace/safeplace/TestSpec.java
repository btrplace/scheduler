    ...
    public void testInstantiate() throws Exception {
        ...
        for (Constraint c : spec.getConstraints()) {
            ...
            switch (c.id()) {
                ...
                case "killed":
                case "root":
                    s = c.instantiate(asList(mo.getMapping().getAllVMs().iterator().next()));
                    break;
                case "preserve":
                    s = c.instantiate(asList(mo.getMapping().getAllVMs().iterator().next(), "cpu", 3));
                    break;
                case "spread":
                case "gather":
                case "lonely":
                    s = c.instantiate(asList(mo.getMapping().getAllVMs()));
                    break;
                case "sequentialVMTransitions":
                    s = c.instantiate(asList(new ArrayList<VM>(mo.getMapping().getAllVMs())));
                    break;
                case "online":
                case "offline":
                case "quarantine":
                    s = c.instantiate(asList(mo.getMapping().getAllNodes().iterator().next()));
                    break;
                case "overbook":
                    s = c.instantiate(asList(mo.getMapping().getAllNodes().iterator().next(), "cpu", 1.5));
                    break;
                case "ban":
                case "fence":
                    s = c.instantiate(asList(mo.getMapping().getAllVMs().iterator().next(), mo.getMapping().getAllNodes()));
                    break;
                case "maxOnline":
                case "runningCapacity":
                    s = c.instantiate(asList(mo.getMapping().getAllNodes(), 3));
                    break;
                case "resourceCapacity":
                    s = c.instantiate(asList(mo.getMapping().getAllNodes(), "cpu", 3));
                    break;
                case "split":
                    s = c.instantiate(asList(
                            asList(mo.getMapping().getReadyVMs(), mo.getMapping().getRunningVMs())
                    ));
                    break;
                case "splitAmong":
                    s = c.instantiate(asList(
                            (List<?>) asList(mo.getMapping().getRunningVMs(), mo.getMapping().getReadyVMs()),
                            (List<?>) asList(mo.getMapping().getOfflineNodes(), mo.getMapping().getOnlineNodes())
                    ));
                    break;
                case "among":
                    s = c.instantiate(asList(
                            mo.getMapping().getAllVMs(),
                            asList(mo.getMapping().getOfflineNodes(), mo.getMapping().getOnlineNodes())
                    ));
                    break;
                default:
